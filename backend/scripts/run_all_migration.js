...
async function runAllMigrations() {
  try {
    ...
    // Set timezone from .env configuration
    ...
    try {
      console.log(`📄 Setting database timezone to: ${timezone} (from .env)...`);
      // SET command doesn't support parameterized queries
      await pool.query(`SET timezone = '${escapedTimezone}'`);
      // Try to set database-level timezone (may require superuser)
      try {
        // ALTER DATABASE doesn't support parameterized queries
        ...
        console.log(`⚠️  Could not set database-level timezone (may require superuser), using session timezone: ${timezone}\n`);
      }
    } catch (error) {
      console.log('⚠️  Could not set timezone:', error.message, '\n');
    }
    ...
    const migrations = [
      ...
      'migration_add_calendar_table.sql',
      'migration_add_calendar_settings.sql',
      'migration_set_timezone.sql'
    ];
    ...
    console.log('   ✅ Added calendar table (dim_calendar) with Sunday as non-working day');
    console.log('   ✅ Added calendar settings (configurable weekends and holidays)');
